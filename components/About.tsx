export default function About() {
  ...
  return (
    <section id="nosotros" aria-labelledby="about-heading" className="py-24 px-6 bg-white text-center">
      <div className="max-w-4xl mx-auto">
        <h2 id="about-heading" className="text-5xl md:text-[56px] font-extrabold mb-6 text-brand-dark leading-tight tracking-tight">
          Sobre Nosotros
        </h2>
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-12 list-none">
          {stats.map((stat) => (
            <li
              key={stat.title}
              className="bg-gray-50 border border-gray-100 border-t-[3px] border-t-brand-red rounded-xl px-6 py-8"
            >
              <p className="text-[52px] font-black text-brand-dark leading-none tracking-tight">
                {stat.num}
        ...
        <ul className="flex flex-wrap justify-center gap-2 list-none" aria-label="Ciudades donde operamos">
          {cities.map((city) => (
            <li
              key={city}
            ...
            >
              <span aria-hidden="true" className="w-1.5 h-1.5 rounded-full bg-brand-red opacity-70" />
              {city}
            </li>
          ))}
        </ul>
